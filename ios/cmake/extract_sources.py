...
SP = os.path.abspath(os.path.join(HERE, "..", "..", "SP"))
...
def make_var(name):
    ...
    out = subprocess.run(cmd, cwd=SP, capture_output=True, text=True)
    if out.returncode != 0:
...
def resolve(obj, build_prefix):
    """Map one object path to a source file relative to SP/code."""
    rel = obj
    ...
    for d in candidates:
        for ext in SRC_EXTS:
            ...
            if os.path.isfile(os.path.join(SP, path)):
                hits.append(path)
                break
    if not hits:
        return None
...
def main():
    ...
    ap.add_argument("-o", "--output",
                    default=os.path.join(HERE, "sources.generated.cmake"))
    args = ap.parse_args()
