...
static void ST_RunBasic( qboolean full )
{
	...
	// setting written to a cvar that is not there: twelve of the twenty-eight
	// it reports on a desktop build. Asked late, the count drops to sixteen and
	// every one that is left is a cvar the build genuinely cannot have. It is
	// also the stronger question, because "is it still the value the launcher
	// set" then gets asked of the code that could have overwritten it.
	//
	// This was briefly moved back here on the belief that it was what made the
	// stick table fail. It was not; see docs/RU/TESTING.md, which records the
	// six runs that settled it and what the table's real fault looks like.
	if ( !full ) {
		ST_CheckLauncherSettings();
